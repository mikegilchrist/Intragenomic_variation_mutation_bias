...
{
   emp.vs.pred <- ggplot(species.df,aes(x=!!sym("Empirical"),y=!!sym(metric))) + geom_pointdensity(size=0.5, show.legend = F) + 
...
calculateIndependentWindow <- function(gc.clust,window.size=20)
{
  roll.avg <- gc.clust %>% group_by(Chr) %>% 
              reframe(`GC3%` = gtools::running(GC3,by=window.size,fun=mean,na.rm=T),
...
target.dir <- paste0("Results/ConstMut/",species,"/
                     run_2/")
...
codonw.files <- paste0("Results/CodonW/", str_replace(species,".cds$",".out_all"))
...
#join with 'gc.df' on species and geneID
df <- df %>% left_join(gc.df, by = c("Species", "GeneID"))

#determine the cluster with the maximum GC3 median
max.cluster <- df %>% group_by(Species, Cluster) %>% 
               summarize(Median.GC3 = median(GC3)) %>% 
               top_n(1) %>%
               dplyr::rename(Max.Cluster = Cluster)

#update cluster assignments based on maximum GC3 median cluster
...
df.long <- df %>% pivot_longer(cols = c(-Species,-GeneID,-Cluster,-Empirical,-GC3,-GC), names_to = "CUB",values_to = "Predicted")
...
This will generate scatter plots similar to those seen in Figures 1 and 2 comparing predicted and empirical expression estimates for the ConstMut and VarMut model. Will also compare predicted expression estimates between the two models.
...
Figure 1 shows the results of the ConstMut model across three genera: Saccharomyces, Candida, Ogataea. Each genera has a species where predicted expression correlates with the empirical estimates of mRNA abundance from kallisto, while the other species show weak or even negative correlations. We also show the distributions of GC3% of the two clusters estimated from the Correspondence Analysis (CA) + CLARA clustering of absolute codon frequencies (see R_scripts/ca.R). The cluster with the highest median GC3% is designated as the Higher GC3% cluster. The difference in median GC3% between the clusters and the results of a KS test are indicated by the brackets. We also show the phylogenetic relationships between the species, units in millions of years.
...
gc.median <- target.species.gc %>% group_by(Species,Cluster) %>% 
             ...
             pivot_wider(id_cols=Species, names_from=Cluster, values_from=c(Median,Min))  %>%
...
gc.sig <- target.species.gc %>%
          ...
          dplyr::select(-GeneID) %>% 
          tidyr::nest(data = GC3) %>% 
          tidyr::pivot_wider(names_from = Cluster, values_from = data) %>% 
          dplyr::mutate(ks.results=purrr::map2(`2`,`1`,~ks.test(.x$GC3,.y$GC3))) %>% 
          dplyr::mutate(tidied=purrr::map(ks.results, tidy),`1` = purrr::map(`1`, nrow),`2` = purrr::map(`2`, nrow)) %>% 
          tidyr::unnest(cols = c(tidied,`1`,`2`)) %>% 
          dplyr::select(-ks.results) %>% 
          dplyr::mutate(p.value=ifelse(p.value == 0,
                                "p < 2.2e-16",
...
compareRSCU <- function(df,species.1,species.2)
{
  ...
  df.sp <- df %>% dplyr::select(codon,!!sym(species.1),!!sym(species.2))
  p <- ggplot(df.sp,aes(x=!!sym(species.1),y=!!sym(species.2))) + geom_point() +
       theme_cowplot() +
       stat_cor(method="spearman",label.sep = "\n") +
...
top5.rscu.wide <- top5.rscu %>% pivot_wider(names_from=Species,values_from = RSCU)
...
high.rscu <- plot_grid(scu.rscu, cpa.rscu, cpd.rscu, ompa.rscu, ompo.rscu, labels=c("A","","B","","C"), label.size=24, nrow=3, ncol=2)
...
Figure 2 compares the results from the ConstMut and VarMut models, with the latter allowing mutation bias parameters to vary for genes in the the different clusters. Species showing greater differences in the median GC3% between the two clusters show improvement in correlations between predicted and empirical gene expression estimates, suggesting the model is doing better at detecting selection on codon usage. For the other species, the correlation is not affected or weakened, the latter consistent with dispersing the signals of selection. We observe that in the case of significant intragenomic mutation bias variation, the predicted expression estimates for ROC-SEMPPR appear to recapitulate the clusters from the CA + CLARA clustering.
...
This figure illustrates the physical clustering of genes assigned to the same clusters determined in the CA + CLARA approach. The left panels quantify this covariation using 20-gene moving averages, while the right panels use non-overlapping windows of 20 genes.
...
lkluy.c[[1]] <- lkluy.c[[1]] + theme(aspect.ratio = 1, plot.margin = margin(10, 10, 10, 10))
...
lkluy.b[[1]] <- lkluy.b[[1]] + theme(aspect.ratio = 1, plot.margin = margin(10, 10, 10, 10))
...
This figure highlights the impact of the VarMut model on identification of the optimal or preferred codon for each amino acid and each species. In the cases where the optimal codon is correctly identified, we still observe large changes in the magnitude of the selection coefficients.
...
Selection coefficients for an amino acid are scaled relative to a single reference codon for an amino acid. Showing the impact of VarMut model on selection coefficient estimates, but this time with all codons rescaled to represent the NNG/C codon relative to the corresponding NNA/T codon. This makes it a little easier to observe the effects of variation in mutation biases on the selection coefficients of G vs. A ending codons or C vs. T ending codons, regardless of which codon is the overall optimal.
...
Mutation bias for an amino acid are scaled relative to a single reference codon for an amino acid. This figure illustrates the difference in mutation bias estimates between the Lower and Higher GC3% clusters, but with all codons rescaled to represent the NNG/C codon relative to the corresponding NNA/T codon.
...
This figure shows the impact of intragenomic variation in mutation bias across 49 Saccharomycotina budding yeasts. Clade names are taken from Labella et al. 2019, Plos Genetics. Results indicate that VarMut model improves the ability to detect selection on codon usage for species showing larger differences in GC3% between the Lower and Higher GC3% clusters.
...
med.diff <- gc.mixture.df %>% 
            ...
            summarize(Median=median(GC3)) %>% 
            pivot_wider(id_cols=Species,names_from=Cluster,values_from=Median) %>%
...
right <- plot_grid(med.diff.plot + theme(legend.position = "none"), nrow=2)

comb <- plot_grid(est.tree, right, ncol=2, labels=c("A","B"))
comb
...
#plot 1: ROC Difference vs. Delta DIC LogLik
dic.loglik.plot <- ggplot(dic.df, aes(x = ROC_Difference, y = Delta.DIC.LogLik, label = Species)) + geom_point() +
            ggrepel::geom_text_repel(point.padding = 5, max.overlaps = Inf) +
            theme_cowplot() +
...
#plot 2: Median GC3 Difference vs. Delta DIC LogLik 
...
corr.deta.dm <- corr.deta.dm %>% 
  mutate(Species=cleanSpeciesLabels(Species)) %>%
  left_join(dic.df, by="Species") %>%
  filter(Delta.DIC.LogLik < 0.0 & ROC_Difference < 0.0) 

#plot 1: ROC_Difference vs. Absolute Correlation
...
p <- ggplot(corr.deta.dm, aes(x=abs(estimate), y=ROC_Difference, label=Species, color=Delta.DIC.LogLik)) + geom_point() +
     ggrepel::geom_text_repel(point.padding = 1, size = 3, box.padding = 1, max.overlaps = Inf, nudge_x = 0.2, nudge_y = -0.1, min.segment.length = 0.2, direction = "both", force = 1.5) +
     theme_cowplot() +
     ylab("Correlation Shift\nVarMut - ConstMut") +
     xlab("|Spearman Correlation Coefficient|\nSelection (ConstMut) - Mutation Difference (VarMut)") + 
  stat_cor(method="spearman", aes(color = NULL), label.y.npc = "bottom", label.x.npc = "center")
p

#plot 2: Delta.DICLogLik vs. Absolute Correlation
p <- ggplot(corr.deta.dm, aes(x=abs(estimate), y=Delta.DIC.LogLik, label=Species, color=ROC_Difference)) + geom_point() +
     ggrepel::geom_text_repel(point.padding = 1, size = 3, box.padding = 2, max.overlaps = Inf, nudge_x = 0.2, nudge_y = -0.1, min.segment.length = 0.2, direction = "both", force = 1.5) +
     theme_cowplot() +
...
runs.k.2 <- paste0("Results/VarMut_3/",species,"/run_2/Parameter_est/gene_expression.txt")
codonw.files <- paste0("Results/CodonW/", str_replace(species,".cds$",".out_all"))
